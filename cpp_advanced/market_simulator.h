...
// ============================================================================
// Generates realistic-ish market data using geometric Brownian motion
// with mean-reverting volatility (simplified Heston-like)
// ============================================================================
...
public:
    MarketSimulator(Price initial_price = 100.0,
                    double volatility   = 0.0015,
                    double drift        = 0.0,
                    double spread_bps   = 2.0,
                    uint64_t tick_interval_ns = 1'000'000,   // 1ms
                    uint32_t seed = 42)
        : rng_(seed)
        , price_(initial_price)
        , volatility_(volatility)
        , base_vol_(volatility)
        , vol_mean_revert_(0.05)
        , vol_of_vol_(0.3)
        , drift_(drift)
    ...
    Tick next_tick() {
        ...
        volatility_ += vol_mean_revert_ * (base_vol_ - volatility_)
                     + vol_of_vol_ * volatility_ * vol_shock;
